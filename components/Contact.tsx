...
const contacts = [
  ...
  },
  {
    icon: Linkedin,
    label: "LinkedIn",
    value: "linkedin.com/in/vinay-adari-261241200",
...
export default function Contact() {
  ...
  return (
    <section id="contact" className="relative py-24 px-6 bg-[#111827]" ref={ref}>
          ...
          {contacts.map((contact) => {
            ...
            return (
              <motion.a
                ...
                className="group glass-card glass-card-hover rounded-2xl p-7 transition-all duration-300"
              >
                <div className="flex items-center gap-4">
                  <div className="w-14 h-14 rounded-xl bg-[#38BDF8]/10 border border-[#38BDF8]/20 flex items-center justify-center">
                    <Icon className="w-7 h-7 text-[#38BDF8] group-hover:text-[#8B5CF6] transition-colors" />
                  </div>

                  <div className="flex-1 min-w-0">
                    <div className="text-xs text-[#9CA3AF] mb-1 uppercase tracking-wider">{contact.label}</div>
                    <div className="text-white text-base font-medium truncate group-hover:text-[#38BDF8] transition-colors">
                      {contact.value}
                    </div>
                  </div>
                </div>
